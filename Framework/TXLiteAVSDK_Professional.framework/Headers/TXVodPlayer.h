...
/**
 * 是否支持 自动切换 Picture In Picture功能（‘自动画中画’功能）
 * 使用自动切换画中画功能，需要当前设备支持画中画功能、打开自动画中画功能（设置-通用-画中画-自动开启画中画）并且开通了高级版license权限[Licence](https://cloud.tencent.com/document/product/881/74588#.E8.B4.AD.E4.B9.B0.E5.B9.B6.E6.96.B0.E5.BB.BA.E6.AD.A3.E5.BC.8F.E7.89.88-license)
 */
+ (BOOL)isSupportSeamlessPictureInPicture;

/**
 * 设置 自动切换 Picture In Picture功能 是否允许
 * 使用画中画能力时需要判断当前设备是否支持
 *
 * @param enabled 自动画中画功能是否允许，YES 表示允许 NO 表示不允许，默认为NO
 */
+ (void)setPictureInPictureSeamlessEnabled:(BOOL)enabled;
